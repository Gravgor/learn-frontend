  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-100 to-pink-200 flex flex-col items-center">
      <header className="bg-gradient-to-r from-pink-400 via-purple-300 to-indigo-300 p-8 text-white shadow-md rounded-b-3xl relative overflow-hidden w-full">
        ...
        <Link
          href={'/'}
          className="absolute top-4 right-4 bg-pink-500 text-white px-4 py-2 rounded-lg hover:bg-pink-600 shadow-md"
        >
          Wróć na stronę główną
        </Link>
      </header>

      <main className="p-6 max-w-8xl mx-auto w-full flex flex-col md:flex-row space-y-6 md:space-y-0 md:space-x-6">
        {lesson ? (
          <>
            <div className="bg-white p-6 rounded-lg shadow-lg space-y-6 flex-1">
              ...
              {/* New Sources Section */}
              <h4 className="text-xl font-semibold text-pink-600 mb-4">Źródła</h4>
              <ul className="list-disc list-inside text-gray-700 space-y-2">
                {lesson.sources.map((source, index) => (
                  <li key={index}>
                    <a href={source} className="text-pink-500 hover:underline" target="_blank" rel="noopener noreferrer">
                      {source}
                    </a>
                  </li>
                ))}
            ...
            {lesson.editor && (
              <div className="bg-white p-6 rounded-lg shadow-lg flex-1">
                ...
                <div className="flex mt-4">
                  ...
                  {lesson.language.toLowerCase() === "javascript" && <span className="text-gray-500 ml-4 mt-2">Kod wykona się w konsoli</span>}
                </div>
